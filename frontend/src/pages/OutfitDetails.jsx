...
const OutfitDetails = () => {
  ...
  const currency = process.env.REACT_APP_CURRENCY || 'USD'; // Default to 'USD' if not defined
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        <form onSubmit={handleSubmit} className="shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500 bg-white">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="date" className="border border-solid border-borderColor px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none" required id="reservation-date" min={new Date().toISOString().split('T')[0]} />
          </div>
          <div className="flex flex-col gap-2">
            <label htmlFor="return-date">Return Date</label>
            <input type="date" className="border border-solid border-borderColor px-3 py-2 rounded-lg" required id="return-date" min={new Date().toISOString().split('T')[0]} />
          </div>
